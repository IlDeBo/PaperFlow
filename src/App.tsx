...
export default function App() {
  const [view, setView] = useState('inventory'); // dashboard, inventory, logs
  ...
  // Stato Dati (Simulazione Database)
  const [inventory, setInventory] = useState<InventoryItem[]>([
    { id: 1, type: "Patinata Opaca", format: "70x100", weight: "150g", stock: 12, unit: "risme", minStock: 5, supplier: "Fedrigoni", location: "A1" },
    { id: 2, type: "Usomano (Offset)", format: "A4", weight: "80g", stock: 3, unit: "risme", minStock: 10, supplier: "Burgo", location: "B2" },
    { id: 3, type: "Kraft Avana", format: "64x88", weight: "250g", stock: 450, unit: "fogli", minStock: 100, supplier: "Favini", location: "C1" },
  ]);

  const [logs, setLogs] = useState<LogEntry[]>([
    { id: 101, itemId: 1, user: "Marco G.", action: "out", qty: "2 risme", item: "Patinata Opaca", date: "Oggi, 10:30" },
    { id: 102, itemId: 2, user: "Admin", action: "in", qty: "50 risme", item: "Usomano 80g", date: "Ieri, 16:45" },
  ]);

  // Helper per Log
  const addLog = (itemId: number, user: string, action: 'in' | 'out' | 'edit' | 'create', qty: string, itemName: string) => {
  ...
  const updateStock = (id: number, delta: number) => {
    ...
    setInventory(prev => prev.map(item => {
      if (item.id === id) {
        ...
        addLog(id, operator, delta > 0 ? 'in' : 'out', `${Math.abs(delta)} ${item.unit}`, item.type);
  ...
  const deleteItem = (id: number) => {
    ...
    addLog(id, deleteOperator, 'out', "Eliminato", itemToDelete.type);
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    addLog(newItem.id, formData.operator, 'create', `${newItem.stock} ${newItem.unit}`, newItem.type);
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    setInventory(prev => prev.map(item => item.id === editData.id ? editData : item));
    
    // Logga il cambiamento di stock se presente
    if (editData.stock !== oldItem.stock) {
      const diff = editData.stock - oldItem.stock;
      addLog(editData.id, editOperator, diff > 0 ? 'in' : 'out', `${Math.abs(diff)} ${editData.unit}`, editData.type);
    } else {
      addLog(editData.id, editOperator, 'edit', "Modifica dettagli", editData.type);
    }
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-[#F9FAFB] text-slate-900 font-sans pb-16 md:pb-0">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-gray-200 p-6 sticky top-0 h-screen">
        ...
        <div className="mt-auto p-4 bg-blue-50 rounded-2xl">
          ...
          <p className="text-sm text-blue-800">Tutti i sistemi sono online</p>
        </div>
      </aside>
          ...
          {view === 'dashboard' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <StatCard label="Movimenti Oggi" value={2} color="text-blue-600" />
              </div>
            </div>
          )}
                    ...
                    <div key={log.id} className={`p-3 sm:p-4 flex items-center justify-between ${i !== 0 ? 'border-t border-gray-50' : ''}`}>
                      <div className="flex items-center gap-3 sm:gap-4">
                        <div className={`p-1.5 sm:p-2 rounded-full ${
                          log.action === 'out' ? 'bg-red-50 text-red-600' : 
                          log.action === 'in' ? 'bg-emerald-50 text-emerald-600' : 
                          log.action === 'create' ? 'bg-blue-50 text-blue-600' :
                          'bg-amber-50 text-amber-600'
                        }`}>
                          {log.action === 'out' ? <ArrowDownLeft size={18}/> : 
                           log.action === 'in' ? <ArrowUpRight size={18}/> : 
                        ...
                        <div>
                          <p className="text-xs sm:text-sm font-semibold">
                            ...
                            <span className="font-normal text-slate-500 hidden sm:inline ml-1">
                              {log.action === 'out' ? (log.qty === 'Eliminato' ? 'ha eliminato' : 'ha prelevato') : 
                               log.action === 'in' ? 'ha caricato' : 
                               log.action === 'create' ? 'ha creato' :
                               'ha modificato'}
                            </span>
                          </p>
                      ...
                      <div className="text-right">
                        <p className={`text-sm sm:text-base font-bold ${
                          log.action === 'out' ? 'text-red-600' : 
                          log.action === 'in' ? 'text-emerald-600' : 
                          log.action === 'create' ? 'text-blue-600' :
                          'text-amber-600'
                        }`}>
                          {log.action === 'edit' ? 'Modifica' : 
                           log.action === 'create' ? 'Nuovo' :
                           `${log.action === 'out' ? '-' : '+'} ${log.qty}`}
                        </p>
      ...
      {/* MODAL INSERIMENTO (Side Panel) */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-sm transition-opacity">
          <div className="w-full sm:max-w-md bg-white h-full shadow-2xl p-6 sm:p-8 animate-in slide-in-from-right duration-300 flex flex-col">
            ...
            <form className="flex-1 flex flex-col overflow-hidden" onSubmit={handleAddItem}>
              <div className="flex-1 overflow-y-auto space-y-5 sm:space-y-6 pr-1">
                <div>
                  ...
                  <div className="mb-3">
                    ...
                    <input 
                      type="text" 
                      placeholder="es. Patinata Opaca" 
                      className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:bg-white focus:border-blue-200 transition-all" 
                      value={formData.type}
                  ...
                  <div className="mt-3">
                    ...
                    <input 
                      type="text" 
                      placeholder="es. Biglietti da visita" 
                      className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:bg-white focus:border-blue-200 transition-all" 
                ...
                <div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2">
                        <input 
                          type="number" 
                          placeholder="0" 
                          className="flex-1 p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:bg-white focus:border-blue-200 transition-all" 
                          value={formData.stock === 0 ? "" : formData.stock}
                          onChange={(e) => setFormData({...formData, stock: e.target.value === "" ? 0 : parseInt(e.target.value) || 0})}
                        />
                        <select 
                          className="bg-gray-50 border border-gray-100 rounded-xl p-3 outline-none focus:bg-white focus:border-blue-200 transition-all"
                          value={formData.unit}
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setFormData({...formData, minStock: e.target.value === "" ? 0 : parseInt(e.target.value) || 0})}
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea 
                    placeholder="Inserisci qui eventuali note o appunti..." 
                    className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:bg-white focus:border-blue-200 transition-all h-24 resize-none" 
              ...
              <div className="pt-6 border-t border-gray-100 mt-auto">
                <button 
                  type="submit"
                  className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-lg shadow-blue-500/30 hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2"
                >
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-sm transition-opacity">
          <div className="w-full sm:max-w-xl bg-white h-full shadow-2xl p-6 sm:p-8 animate-in slide-in-from-right duration-300 flex flex-col">
              ...
              {isEditing ? (
                <form id="editForm" onSubmit={handleSaveEdit} className="space-y-6">
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e) => setEditData(prev => prev ? {...prev, format: e.target.value} : null)}
                          required
                        />
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e) => setEditData(prev => prev ? {...prev, color: e.target.value} : null)}
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold uppercase text-slate-400 mb-1">Finitura</label>
                        <input 
                          ...
                          onChange={(e) => setEditData(prev => prev ? {...prev, finish: e.target.value} : null)}
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <input 
                        ...
                        onChange={(e) => setEditData(prev => prev ? {...prev, usage: e.target.value} : null)}
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e) => setEditData(prev => prev ? {...prev, stock: e.target.value === "" ? 0 : parseInt(e.target.value) || 0} : null)}
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold uppercase text-slate-400 mb-1">Soglia Minima (Alert)</label>
                        <input 
                          type="number" 
                          className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl outline-none"
                          value={editData?.minStock === 0 ? "" : editData?.minStock}
                          onChange={(e) => setEditData(prev => prev ? {...prev, minStock: e.target.value === "" ? 0 : parseInt(e.target.value) || 0} : null)}
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e) => setEditData(prev => prev ? {...prev, supplier: e.target.value} : null)}
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold uppercase text-slate-400 mb-1">Posizione</label>
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-slate-400 mb-1">Note e Appunti</label>
                      <textarea 
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-slate-400 mb-1">Operatore (Richiesto per salvare)</label>
                      <input 
                        type="text" 
                        placeholder="Tuo nome"
                        className="w-full p-3 bg-blue-50 border border-blue-100 rounded-xl outline-none focus:bg-white focus:border-blue-300"
                        value={editOperator}
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-slate-500 font-medium">
                          ...
                          {selectedItem.finish && ` • ${selectedItem.finish}`}
                          {selectedItem.color && ` • ${selectedItem.color}`}
                          {` • Posizione: ${selectedItem.location}`}
                        </p>
                        <div className="mt-4 grid grid-cols-2 gap-4">
                          {selectedItem.color && (
                            <div>
                              <p className="text-[9px] uppercase font-bold text-slate-400">Colore</p>
                              <p className="text-sm font-bold text-slate-700">{selectedItem.color}</p>
                            </div>
                          )}
                          {selectedItem.finish && (
                            <div>
                              <p className="text-[9px] uppercase font-bold text-slate-400">Finitura</p>
                              <p className="text-sm font-bold text-slate-700">{selectedItem.finish}</p>
                            </div>
                          )}
                          {selectedItem.usage && (
                            <div className="col-span-2">
                              <p className="text-[9px] uppercase font-bold text-slate-400">Uso Previsto</p>
                              <p className="text-sm font-bold text-slate-700">{selectedItem.usage}</p>
                            </div>
                          )}
                          {selectedItem.notes && (
                            <div className="col-span-2 pt-2 border-t border-slate-200">
                              <p className="text-[9px] uppercase font-bold text-slate-400">Note</p>
                              <p className="text-xs text-slate-600 italic whitespace-pre-wrap">{selectedItem.notes}</p>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              {isEditing ? (
                <div className="grid grid-cols-2 gap-3">
                  <button 
                    ...
                    className="py-4 bg-gray-100 text-slate-600 rounded-2xl font-bold hover:bg-gray-200 transition-all"
                  >
                    Annulla
                  </button>
                  <button 
                    form="editForm"
                    type="submit"
                    className="py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-lg shadow-blue-500/30 hover:bg-blue-700 transition-all"
                  >
                    Salva Modifiche
                  </button>
                </div>
              ) : isDeletingConfirm ? (
                <div className="bg-red-50 p-6 rounded-3xl border border-red-100 animate-in fade-in zoom-in duration-200">
                  <p className="text-sm font-bold text-red-700 mb-4 text-center">Conferma Eliminazione Definitiva</p>
                  <input 
                    type="text" 
                    placeholder="Tuo nome per confermare"
                    className="w-full p-3 bg-white border border-red-200 rounded-xl outline-none mb-3 text-sm"
                    value={deleteOperator}
                    onChange={(e) => setDeleteOperator(e.target.value)}
                  />
                  <div className="grid grid-cols-2 gap-3">
                    <button 
                      onClick={() => { setIsDeletingConfirm(false); setDeleteOperator(""); }}
                      className="py-3 bg-white text-slate-600 border border-gray-200 rounded-xl font-bold text-sm"
                    >
                      Annulla
                    </button>
                    <button 
                      onClick={() => deleteItem(selectedItem.id)}
                      className="py-3 bg-red-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-red-500/30"
                    >
                      Elimina Ora
                    </button>
                  </div>
...
function MobileNavItem({ icon, label, active, onClick }: NavItemProps) {
  return (
    <button 
      onClick={onClick}
      className={`flex flex-col items-center justify-center flex-1 py-1 transition-all ${active ? 'text-blue-600' : 'text-slate-400'}`}
    >
      {icon}
...
function NavItem({ icon, label, active, onClick }: NavItemProps) {
  return (
    <button 
      onClick={onClick}
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${active ? 'bg-blue-50 text-blue-600 font-bold' : 'text-slate-500 hover:bg-gray-50 hover:text-slate-900'}`}
    >
      {icon}
...
function InventoryCard({ item, onUpdate, onClick }: InventoryCardProps) {
  ...
  return (
    <div 
      onClick={onClick}
      className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-all group cursor-pointer active:scale-[0.98]"
    >
